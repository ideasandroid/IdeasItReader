...
public class WebImageCache extends
	...
	public void handleImageView(final ImageView image, final String url,
			String tag,final float scale) throws Exception {
		Bundle message = ((SimpleBus) getBus()).createMessage(url);

		Drawable d = get(url, message);
		Matrix mMatrix=new Matrix();
		Bitmap oldbitmap=drawableToBitmap(d);
		mMatrix.postScale(scale, scale);
		//Bitmap.createScaledBitmap(ImageUtil.getNetBitmap(url), 90, null, true);
		final Bitmap b = Bitmap.createBitmap(oldbitmap, 0, 0, oldbitmap.getWidth(), oldbitmap.getHeight(),mMatrix,true);
		
		image.setImageDrawable(new BitmapDrawable(b));
							...
							.runOnUiThread(new Runnable() {
								public void run() {
									if (url.equals(message
											.getString(SimpleBus.KEY))) {
										Matrix mMatrix=new Matrix();
										Bitmap oldbitmap=drawableToBitmap(get(url, null));
										mMatrix.postScale(scale, scale);
										//Bitmap.createScaledBitmap(ImageUtil.getNetBitmap(url), 90, null, true);
										final Bitmap b = Bitmap.createBitmap(oldbitmap, 0, 0, oldbitmap.getWidth(), oldbitmap.getHeight(),mMatrix,true);
										image.setImageDrawable(new BitmapDrawable(b));
	...
	public void handleImageSwitcher(final ImageSwitcher image, final String url,
			String tag) throws Exception {
		Bundle message = ((SimpleBus) getBus()).createMessage(url);
	...
	class FetchImageTask extends AsyncTaskEx<Object, Void, Void> {
		...
		protected Void doInBackground(Object... params) {
			...
			try {
				...
				WebImageCache.this.put(url, new BitmapDrawable(
